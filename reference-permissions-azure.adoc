...
summary: When the NetApp Console launches a Console agent in Azure, it attaches a custom role to the VM that provides the agent with permissions to manage resources and processes within that Azure subscription. The agent uses the permissions to make API calls to several Azure services.
---
...
** link:task-install-agent-azure-marketplace.html#step-3-set-up-permissions[Set up permissions for an Azure Marketplace deployment]
...
The following JSON gives an example of a custom role for Backup and Recovery that uses the fewest possible permissions and the narrowest scope possible:
...
=== Legacy indexing permissions

The agent makes the following API requests when you use the Search & Restore functionality. You only need these lpermissions if you enabled legacy indexing features before the release of indexing v2:
...
The following permissions were removed from the JSON policy so that the policy can reflect the minimum required permissions for Backup and Recovery:
