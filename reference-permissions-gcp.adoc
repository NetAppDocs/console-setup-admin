...
//The `cloudbuild.workerpools.get`, `cloudbuild.workerpools.list`, and `compute.forwardingRules.update` permissions are added for supporting Infrastructure Manager in Cloud Volumes ONTAP deployments in Google Cloud. The `cloudbuild.workerpools.get` and `cloudbuild.workerpools.list` permissions are only required for private mode Cloud Volumes ONTAP deployments supporting Infrastructure Manager.
