...
| cloudbuild.workerpools.get| To access worker pool information during deployment of Cloud Volumes ONTAP systems in private mode. | Yes | Yes | Yes
| cloudbuild.workerpools.list | To list worker pools during deployment of Cloud Volumes ONTAP systems in private mode. | Yes | No | No
...
The `compute.forwardingRules.update`, `cloudbuild.workerpools.list`, and `cloudbuild.workerpools.get` permissions are added to facilitate SVM-based management of the Cloud Volumes ONTAP systems converted from Google Cloud Deployment Manager to Google Cloud Infrastructure Manager. The `cloudbuild.workerpools.list` and `cloudbuild.workerpools.get` permissions are only required for private mode deployments needing transition from Deployment Manager to Infrastructure Manager.
