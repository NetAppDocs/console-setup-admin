...
| compute.firewalls.create .8+| To create firewall rules for Cloud Volumes ONTAP. | Yes | No | No
...
| logging.logEntries.list .3+| To get stack log drives. | Yes | Yes | No
...
| compute.instances.setServiceAccount .8+| To set a service account on the Cloud Volumes ONTAP instance. This service account provides permissions for data tiering to a Google Cloud Storage bucket. | Yes | Yes | No
...
| compute.backendServices.create .6+| To configure a backend service for distributing traffic in an HA pair. | Yes | No | No
