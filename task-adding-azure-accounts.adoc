...
image:diagram_azure_authentication.png["Conceptual image that shows the Console obtaining authentication and authorization from Microsoft Entra ID before it can make an API call. The Console role defines permissions in Active Directory. It is tied to one or more Azure subscriptions and a service principal object that represents the Console application."]
