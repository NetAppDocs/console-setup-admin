...
You must use IAM user access keys if the Console agent is installed on-premises. You can't use an IAM role.
...
. Upload the self-signed certificate to your trusted certificates if your environment requires it. You can replace this certificate after installation. link:task-installing-https-cert.html[Learn how to replace the self-signed certificate.]
...
If the installation fails, you can access the validation report(JSON format) and the configuration logs directly from the Console agent host in the following directories:
