...
[.lead]
If you are a VMWare user, you can use an OVA to install a Console agent in your VCenter. 

NOTE: When you install a Console agent with your VCenter tool, you can use the VM web console to perform maintenance tasks. link:task-agent-vm-config.html[Learn more about the VM console for the agent.]


== Prepare to install the Console agent
Before you install, ensure your VM host meets requirements and ensure that the Console agent will have outbound access to the internet as well as targeted networks. To use NetApp data services or Cloud Volumes ONTAP, create cloud provider credentials for the Console agent to perform actions on your behalf.
...
TIP: Google Cloud is not supported because the Console agent installed on your premises cannot manage resources in Google Cloud.
...
NetApp supports installing the Console agent in your VCenter environment.
...
. Select the network to which you want to connect the VM, then select *Next*. The network must be IPv4 and have outbound internet access to the required endpoints.

. in the *Customize template* window, complete the following fields:

+
* Proxy information: If you selected explicit proxy, enter the proxy server hostname or IP address and port number, as well as the username, password. If you selected transparent proxy, upload the respective certificate.
* Virtual Machine Configuration
** Skip config check: This is unchecked by default. Mark this checkbox if you want to install without running a configuration check. NetApp recommends leaving this box unchecked. The Configuration check validates that the agent has network access to the required endpoints. If it deployment fails because of connectivity issues, you can access the validation report and logs from the agent host.
...
. Power on the VM if you didn't select the option to automatically power it on.

=== Access the report and logs from the Console agent host

If the installation fails, you can access the validation report in JSON format and the configuration logs directly from the Console agent host in the following directories:
