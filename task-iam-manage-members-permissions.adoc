...
Note the following important points about granting access in NetApp Console:

* All users regardless of how they are added to the organization must first sign up for the NetApp Console before they can be granted access to resources. 
* Users must have at least one explicitly assigned role in the Console before they can access resources, even if they are members of a federated group that has been assigned a role.   
...
You can add two types of members to your organization: a user account and a service account. Applications use service accounts to do API tasks automatically. People use user accounts to log in and manage resources.
...
+
Once users sign up, they complete the *Sign up* page, check their email, and log in. If prompted to create an organization, users should close it and notify an Org admin. link:task-user-settings.html#contact-admin[Learn how to contact an organization administrator.^]
+
...
. Optional: Select an additional role or project. To give access to more folders, projects, or roles, select *Add role*, choose the folder, project, or role category, and select a role.
...
. elect a *Category* then select a *Role* that gives the member permissions for the resources in the organization, folder, or project you selected.
