...
= Deploy a NetApp Console agent from the AWS Marketplace
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
You deploy a Console agent in AWS directly from the AWS Marketplace. To deploy a Console agent from the AWS Marketplace, you need to set up your networking, prepare AWS permissions, review instance requirements, and then deploy the Console agent.

Deploying an agent in AWS launches an EC2 instance running Linux and the Console agent software in a VPC of your choice.

.Before you begin

* link:concept-agent-install-options.html#agent-marketplace-deploy[Learn when to deploy from the AWS Marketplace].
...
include::_include/endpoints-agent.adoc[tag=upgrade-endpoints-saas]
...
Creating the Console agent from the AWS Marketplace deploys an EC2 instance in AWS using a default configuration. link:reference-agent-default-config.html[Learn about the default configuration for the Console agent].
...
If you have Amazon S3 buckets in the same AWS account where you created the Console agent, you'll see an Amazon S3 working environment appear on the *Systems* page automatically. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html[Learn how to manage S3 buckets from NetApp Console^]