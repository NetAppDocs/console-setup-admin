...
= Configure a NetApp Console agent to use a proxy server
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
IMPORTANT: Once you have configured a proxy, you cannot change the proxy type. If you need to change the proxy type, you must remove the Console agent and add a new agent with the new proxy type.
...
You can update a Console agent's existing transparent proxy server by using the `proxy update` command or remove the transparent proxy server by using the `proxy remove` command. For more information, review the documentation for link:reference-agent-maint-console-proxy.html[Agent maintenance console].
...
If you configured a Console agent to use a proxy server, you can enable direct API traffic on the Console agent to send API calls directly to cloud provider services without going through the proxy. Agents running in AWS, Azure, or Google Cloud support this option.
