...
Choose one or more services to validate. Valid service names are: `PLATFORM`.



`--validationTypes <comma-separated list validation types to run>` --*REQUIRED*--
[role="quick-margin-para"]
Choose from one or more validation types to run. Valid validation types are: `NETWORK`.
