...
+
If you use a Standard SKU IP address instead, then the Console uses the _private_ IP address of the Console agent, instead of the public IP. If the machine that you're using to access the Console doesn't have access to that private IP address, then actions from the Console fail.
+
