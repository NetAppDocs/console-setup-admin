...
= Deploy the NetApp Console agent in restricted mode
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
+
link:task-prepare-restricted-mode.html#step-3-review-host-requirements[Review agent requirements].
...
. Follow the prompts to set up and start the instance:
...
+
If you use a Standard SKU IP address instead, then the Console uses the _private_ IP address of the Console agent, instead of the public IP. If the machine you use to access the Console cannot reach the private IP address, the Console does not work.
+
...
+
A managed identity lets the Console agent VM identify itself to Microsoft Entra ID without credentials. https://docs.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/overview[Learn more about managed identities for Azure resources^].
...
* You need to disable the configuration check that verifies outbound connectivity during installation. The manual install fails if this check is not disabled. link:task-troubleshoot-agent.html[Learn how to disable configuration checks for manual installations.]
...
--
Provide the NetApp Console with the credentials for the Azure service principal that you previously set up.
