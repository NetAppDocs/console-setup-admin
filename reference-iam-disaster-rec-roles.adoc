...
* *Disaster recovery failover admin*: Perform failovers and migrations. 
* *Disaster recovery application admin*: Create and modify replication plans; initiate test failovers.
* *Disaster recovery viewer*: View information only.

The following table indicates the actions that each role can perform. 

[cols="30,20a,20a,20a,20a",options="header",width="100%"]
|===
| Action
...
5+| *Activation and discovery*
| Activate license | Yes | No | Yes | No
| Initiate free trial | Yes | No | No | No
...
| View credentials | Yes | Yes | No | No
| Create, update, or delete credentials | Yes | Yes | No | No

5+| *Sites*
| View sites | Yes | Yes | Yes | Yes
| Add, modify, or delete sites or clusters | Yes | No| No | No
| View clusters | Yes | No | No | No
...
5+| *Settings*
| Create or modify AppVaults | Yes | Yes | No | No
| Create or modify resource transformations | Yes | Yes | No | No
