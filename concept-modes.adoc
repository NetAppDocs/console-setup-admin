...
Private mode (installed on your premises)::
Private mode serves organizations with the strictest security and sovereignty requirements, including those operating in AWS Secret Cloud, AWS Top Secret Cloud, Azure IL6, or completely air-gapped environments. Private mode has no connection requirements to the NetApp Console SaaS application (where additional API endpoints are hosted), so you have full control over your data and follow rules that block outside communication. Available services focus on backup and recovery, classification, replication, and Cloud Volumes ONTAP. All storage and authentication remain local, keeping data within your environment. This mode is suggested for defense contractors, financial institutions with strict data residency rules, and government agencies with classified systems.
...
| Alerts | Yes
| Amazon FSx for ONTAP | Yes for discovery and management. Discovery without a Console agent (direct discovery) is also supported, but management features aren't available without an agent.
| Amazon S3 | Yes
| Azure Blob | No
...
| On-premises ONTAP clusters | Required for discovery and management. Discovery without a Console agent (direct discovery) is also supported, but management features aren't available without an agent.
| StorageGRID | Yes
| Workloads | No
...
| Licenses and subscriptions management | N/A
| Manage cloud provider credentials | N/A 
| Multi-factor authentication (local users) | N/A
| Notifications | N/A
| NSS accounts | N/A
| Organization Partnerships | N/A
| Read-only mode | N/A
| Notifications | N/A
...
| On-premises ONTAP clusters | Discovery with a CConsole agent and discovery without a Console agent (direct discovery) are both supported. 
...
The following table lists the storage management features, data services, and administrative features available in NetApp Console when you install it in private mode.
...
| Audit | N/A
| Licenses and subscriptions management (You can track the supported licensing options listed below for private mode.)| N/A
| Identity and access management | N/A
| Manage cloud provider credentials | N/A
| Read-only mode | N/A
| Notifications | N/A
...
Private mode (installed in your own secure cloud region or on premises)::
You install a Console agent on-premises or in the cloud (in a secure region, sovereign cloud region, or commercial region) and has _no_ connectivity to the NetApp Console API endpoints (hosted by the SaaS application). You access the local UI provided by the Console agent, not the SaaS application.
