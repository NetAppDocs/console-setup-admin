...
NOTE: Add your folders and projects before assigning users access. link:task-iam-manage-folders-projects.html[Learn how to add folders and projects.]

== Understand how access is granted in NetApp Console
The NetApp Console uses a role-based access control (RBAC) model to manage user permissions. You can assign predefined roles to members individually or through federated groups. You can add and assign roles to service accounts, as well as federated groups. Each role defines what actions a member can perform on the associated resources.
...
[#manage-permissions]
== View role(s) assigned to a member

You can verify which roles the member is currently assigned.
