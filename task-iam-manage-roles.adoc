...
Users with the *Organization admin* or *Folder or project admin* role have the responsibility of assigning roles to other users. You can assign access roles on a project or folder basis. For example, you can assign a user the Ransomware Resilience admin role for one project and the SnapCenter admin role for a different project. Alternatively, if a user needs the Ransomware Resilience admin role for all projects within a specific folder, you can give them this role at the folder level.
