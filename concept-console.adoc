...
summary: NetApp Console offers a SaaS experience for full feature access or a self-hosted option for organizations with connectivity restrictions.
---
...
[.lead]
NetApp Console offers a SaaS experience for full feature access or a self-hosted option for organizations with connectivity restrictions.

You can use NetApp Console in two ways: as a fully managed SaaS experience or as a self-hosted solution in your own cloud environment. With the SaaS option, you access the Console through console.netapp.com while NetApp manages all infrastructure, authentication, and updates. You get complete feature access directly from your web browser. If you choose the self-hosted option, you install a Console agent in your AWS, Azure, or Google Cloud environment. The interface runs locally, and the agent maintains limited outbound connectivity to NetApp for essential tasks like authentication and updates.

Choose the SaaS experience if your organization has standard internet connectivity. You benefit from the simplest setup, automatic updates, and full access to all NetApp storage systems, data services, and advanced features. You can add agents directly from the Console, and NetApp handles all maintenance.

Select the self-hosted option if your organization requires strict connectivity, security, or compliance controls, such as in government or regulated industries. You gain more control and network isolation, but you can only manage select NetApp storage systems and use limited data services, with some features unavailable. You install a Console agent manually and access the Console directly from the local agent interface. Install your agent through your cloud marketplace or manual installation, and the agent communicates outbound-only to NetApp, ensuring no inbound internet connections.
...
image:diagram-standard-mode.png["A conceptual image that shows the architecture of standard mode with the public internet where the SaaS layer and auth0 authentication are available."]
...
image:diagram-restricted-mode.png["A conceptual image of restricted mode."]
