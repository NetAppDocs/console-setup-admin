...
By default, users are not prompted to enter their ONTAP credentials to access ONTAP System Manager. However, a Console administrator (with the Organization admin role) can configure the Console to prompt users to enter their ONTAP credentials. When this setting is enabled, users need to enter their ONTAP credentials each time.
