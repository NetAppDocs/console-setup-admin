...
.6+| Manage EBS volumes that Cloud Volumes ONTAP uses as backend storage
...
.6+| Create and manage stacks for AWS CloudFormation templates
...
| cloudformation:ListStacks | Yes | Yes | No | No
...
.3+| Enable data encryption of Cloud Volumes ONTAP using the AWS Key Management Service (KMS)
//| kms:List* | Yes | Yes | No
...
The following permission is now required for Data Classification: 
