...
* link:task-maintain-agents.html#connect-linux-host[Check the Console agent status (for Linux host deployments)]
...
docker ps -a
----
Podman (for Red Hat Enterprise Linux deployments):: 
+
[source,cli]
----
podman ps -a
...
IMPORTANT: For government cloud customers (restricted mode), NetApp Console does not validate the restricted mode upgrade endpoint. You must manually confirm that your firewall rules allow access to this endpoint.link:task-prepare-restricted-mode.html[Learn more about required network access points for restricted mode.]
