...
= Prepare for deployment in NetApp Console private mode
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
+
With private mode, the only time that NetApp Console sends outbound traffic is to your cloud provider to create a Cloud Volumes ONTAP system.
...
If the Console agent is installed in the cloud and you plan to create Cloud Volumes ONTAP systems, NetApp Console requires cloud provider permissions. You need to set up permissions in your cloud provider and then associate those permissions with the Console agent instance after you install it.
...
Your service principal is now set up and you should have copied the application (client) ID, the directory (tenant) ID, and the value of the client secret. Enter this information in NetApp Console when you add an Azure credential.
--
