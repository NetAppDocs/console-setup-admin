...
. `deploymentmanager.editor` is only required at the host project .if you are not passing firewall rules to the deployment and choose to let the Console create them for you. The NetApp Console creates a deployment in the host project which contains the VPC0 firewall rule if no rule is specified.
...
. For Cloud Tiering, the tiering service account must have the `serviceAccount.user` role on the service account, not just at the project level. If you assign `serviceAccount.user` at the project level, the permissions don't show when you query the service account with `getIAMPolicy`.
