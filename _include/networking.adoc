...
As you use the web-based NetApp Console SaaS application, it contacts several endpoints to complete data management tasks. This includes endpoints that are contacted to deploy the Console agent from the Console.
+
...
//tag::ports[]
Ports::
There's no incoming traffic to the Console agent, unless you initiate it or it is used as a proxy to send AutoSupport messages from Cloud Volumes ONTAP to NetApp Support.
