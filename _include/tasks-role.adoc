...
//tag::add-role[]
. Select the actions menu image:icon-action.png[an icon with three vertical dots] next to the member to whom you want to assign a role and select *Add a role*.
...
.. To unassign a member's role, select image:icon-delete.png[An icon that resembles a trash can] next to the role to remove the respective role from the member. You'll be asked to confirm the removal.
