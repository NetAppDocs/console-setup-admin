 ...
 * For new agent deployments, NetApp checks for these endpoints. This configuration check fails with an error if you are using the previous endpoints used for upgrades,  link:reference-networking-saas-console-prev.html[listed here^]. Although your agent can still receive upgrades with the previous endpoints, NetApp recommends updating your firewall rules to allow access to the current endpoints and block access to the previous endpoints.
