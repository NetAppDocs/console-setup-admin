...
The validation check does not include the restricted mode upgrade endpoints. If your agent is in a restricted mode environment, you must manually confirm that your firewall rules allow access to the appropriate upgrade endpoints for your environment. 
+
...
//the below tag is not in use

//tag::upgrade-endpoints-explanation[]

//Same text is also in endpoints-manual-install but this text is obsolete
...
// the below tag is no longer in use
