...
* Install python3 
* Install the respective podman-compose package
* Add podman-compose to the PATH environment variable
* Verify that your Podman version is using Netavark Aardvark DNS instead of CNI
...
. Install Podman.
+
Get Podman from Red Hat Enterprise Linux repositories.
+
...
----
. Install EPEL to get podman-compose.


.. If using Red Hat Enterprise 9.1 to 9.6:
+
[source,cli]
----
...
+
Install a supported Docker Engine version by following the steps. Do not install the latest version, as the Console does not support it.
