...
= Create a Console agent in Google Cloud from Google Cloud
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* You should have a link:concept-connectors.html[understanding of Console agents].
...
Creating the Console agent deploys a VM instance in Google Cloud with the default configuration. Do not switch to a smaller VM instance with fewer CPUs or less RAM after you create the Console agent. link:reference-connector-default-config.html[Learn about the default configuration for the Console agent].
